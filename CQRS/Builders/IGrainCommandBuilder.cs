...
public interface IGrainCommandBuilder<T, C, Q, K> : IProduceUniversalBuilder<T, K>, IRunBuilder<T, Task<K>> where T : IGrain
{
    ...
    public IGrainCommandBuilder<T, C, Q, IEnumerable<Output>> ForEachTell<Handler, Input, Output>
    (
...
{
    public static IGrainCommandBuilder<T, C, Q, IEnumerable<Output>> ThenForEachTell<T, C, Q, Command, Input, Output>
    (
        this IGrainCommandBuilder<T, C, Q, IEnumerable<Input>> target
    )
        where T : IGrain
        where Command : IAsyncCommandHandler<T, Input, Output>, C
    {
        return new GrainCommandBuilder<T, C, Q, IEnumerable<Output>>(
            target.GrainFactory,
            target.ResolveChildGrain,
            target.Builder.ThenForEachTell<T, Command, Input, Output>()
        );
    }

    public static IGrainCommandBuilder<T, C, Q, IEnumerable<Output>> ThenForEachSplit<T, C, Q, Input, Output>
    (
        this IGrainCommandBuilder<T, C, Q, IEnumerable<Input>> target,
        Func<IGrainCommandBuilder<T, C, Q, Input>, IGrainCommandBuilder<T, C, Q, Output>> operation
    )
